...
// List gets list the resource of the given kind in the given namespace with the given name.
func (v *resourceVerber) List(kind string, namespace string) (*unstructured.UnstructuredList, error) {
	gvr, err := v.groupVersionResourceFromKind(kind)
	if err != nil {
		return nil, err
	}
	return v.client.Resource(gvr).Namespace(namespace).List(context.TODO(), helpers.ListEverything)
}
