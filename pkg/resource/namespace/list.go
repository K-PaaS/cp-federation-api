...
func GetNamespaceList(client kubernetes.Interface, dsQuery *dataselect.DataSelectQuery) (*CustomNamespaceList, error) {
...
func toNamespaceList(namespaces []v1.Namespace, nonCriticalErrors []error, dsQuery *dataselect.DataSelectQuery) *CustomNamespaceList {
	//namespaces = common.FilterNamespaceByArrayNamespace(namespaces)
	namespaceList := &CustomNamespaceList{
		Namespaces: make([]CustomNamespace, 0),
	...
	for _, namespace := range namespaces {
		namespaceList.Namespaces = append(namespaceList.Namespaces, toCustomNamespace(namespace))
	}
