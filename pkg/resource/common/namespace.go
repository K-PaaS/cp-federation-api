...
func FilterNamespaceByArrayNamespace(namespaces []api.Namespace) []api.Namespace {
	filtered := make([]api.Namespace, 0, len(namespaces))
	for _, namespace := range namespaces {
		matched := IsFilterNamespaceCheck(namespace.Name)
		if !matched {
...
func FilterNamespaceByArrayString(namespaces []string) []string {
	filtered := make([]string, 0, len(namespaces))
	for _, namespace := range namespaces {
		matched := IsFilterNamespaceCheck(namespace)
		if !matched {
			filtered = append(filtered, namespace)
		}
	}
...
func IsFilterNamespaceCheck(item string) bool {
